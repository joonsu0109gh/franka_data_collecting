...
os.makedirs(f'{current_dir}/data/train', exist_ok=True)
for i in tqdm.tqdm(range(N_TRAIN_EPISODES)):
    create_fake_episode(f'{current_dir}/data/train/episode_{i}.npy')

print("Generating val examples...")
os.makedirs(f'{current_dir}/data/val', exist_ok=True)
for i in tqdm.tqdm(range(N_VAL_EPISODES)):
    create_fake_episode(f'{current_dir}/data/val/episode_{i}.npy')
